...
Each card's title is the session name if one was reported, falling back to the session ID — and, when a recent dev-workflow checkpoint has the session's own repo at a non-terminal stage (checkpoints older than about a week are ignored), a second line showing that checkpoint's stage(s), one `repo:stage` entry per line for a multi-repo story. Cards sort needs-attention first, show time in state, the latest message snippet, and last-update age, refresh every 3 seconds, and have a per-card dismiss control for crashed/abandoned sessions. Sessions silent for over 24 hours are pruned automatically.

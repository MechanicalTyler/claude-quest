...
- If user asks a question rather than approving or requesting a change: answer the question directly (using the same codebase/context investigation Phase 3 already permits), then re-present the identical, unmodified draft with the same approval prompt. This does not count as a revision cycle — do not increment the 3-revision-cycle counter used by the feedback branch above.

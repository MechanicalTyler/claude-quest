...
- **Hard-fail rules have no override path** — Some documented rules are deterministic and non-negotiable; asking the user is not a way around them:
  ...
  - The **Loop Safety Guard**'s stop-after-3 cycle cap has no config-driven exemption list, and cannot be silently bypassed, inferred from context, or waived by a subagent or orchestrator acting on its own. Its sole exception is a fresh, explicit instruction from the human user, given in the current session, authorizing additional cycles beyond the cap — that specific extension only, every time it is invoked, never a standing waiver for future runs. "Ask first" for it means: when the cap is hit with no such fresh instruction already given, stop and report that the cycle cap was reached, and ask whether the user wants to authorize more cycles

...
**`reviewing-prs` and `testing-prs` are also named call sites, for a narrower purpose: local verification, not implementation.** Neither stage implements or commits code, but both may decide to run a local verification command (build, lint, test) against a PR's code before or alongside CI. Before running any such command, both stages MUST perform the same live lookup as `dev-workflow-fixer.md` — `git worktree list --porcelain` scoped to the target repo, matching the PR's branch — and reuse the matching worktree if one exists. If no matching worktree exists, their fallback is **never** the primary checkout: create a new scratch worktree via `superpowers:using-git-worktrees` for the PR's branch, run the verification command(s) there, then remove that scratch worktree immediately afterward (unlike the developer/fixer worktrees, which persist until a pipeline-level reclamation point — see "Cleanup" below). This fallback deliberately differs from `dev-workflow-fixer.md`'s own fallback (a plain `gh pr checkout {PR_NUMBER}` in the primary checkout): `fixer` needs to commit and push to that branch, so occupying the primary checkout is an accepted trade-off for that role; `reviewing-prs`/`testing-prs` are read-only and have no reason to touch the primary checkout's branch at all. This section exists because a reviewer subagent once checked out a temp branch directly in a primary checkout with no documented guardrail and could not switch it back, stranding it.

#!/usr/bin/env -S uv run --script
# /// script
# requires-python = ">=3.8"
# dependencies = []
# ///

import json
import sys
from pathlib import Path

try:
    from subagent_tracker import mark_subagent_active, mark_background_active
except ImportError:
    import importlib.util
    tracker_spec = importlib.util.spec_from_file_location(
        "subagent_tracker",
        Path(__file__).parent / "subagent_tracker.py"
    )
    subagent_tracker = importlib.util.module_from_spec(tracker_spec)
    tracker_spec.loader.exec_module(subagent_tracker)
    mark_subagent_active = subagent_tracker.mark_subagent_active
    mark_background_active = subagent_tracker.mark_background_active


def _bash_background_label(tool_input):
    description = str(tool_input.get("description") or "").strip()
    if description:
        return description
    return str(tool_input.get("command") or "").strip()[:100]


def main():
    # PreToolUse: pure observer, not a gate. Must always exit 0 and never
    # print a permission-decision payload.
    try:
        input_data = json.load(sys.stdin)
        tool_name = input_data.get("tool_name")
        tool_input = input_data.get("tool_input")
        if not isinstance(tool_input, dict):
            tool_input = {}
        session_id = input_data.get("session_id", "")

        if tool_name == "Agent":
            label = str(tool_input.get("description") or "").strip()
            mark_subagent_active(session_id, label=label)
        elif tool_name == "Bash" and tool_input.get("run_in_background"):
            mark_background_active(session_id, "bash", _bash_background_label(tool_input))
    except Exception:
        pass
    sys.exit(0)


if __name__ == "__main__":
    main()

...
def active_subagent_home(tmp_path, monkeypatch):
    ...
    logs) never touch the real ~/.claude. Markers live under notifications'
    own namespace now (subagent_tracker.py) -- fully independent of
    attention-hub. Returns the active-subagents directory path (parent of
    each session's own marker subdirectory)."""

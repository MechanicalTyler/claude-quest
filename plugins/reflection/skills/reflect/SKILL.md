...
- **Groups fully written up with a specific, nameable root cause:** do NOT set `status: reported` yet. Bundle these findings before asking about remediation:
  ...
  A root-caused finding is only closed out once its bundle is linked to a story or explicitly declined — a report file existing is never, by itself, grounds to mark it `reported`.

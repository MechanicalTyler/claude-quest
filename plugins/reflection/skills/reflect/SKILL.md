...
Tell the user the file path. In interactive mode, also ask whether they'd like to act on any suggestion now — as a separate, explicit follow-up action outside this skill's own scope (this skill does not implement the fix itself).

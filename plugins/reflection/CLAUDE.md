...
**reflection** is a standalone Claude plugin (v0.3.0) that tracks agent-performance inconsistencies across sessions. It is fully decoupled from `dev-workflow` — it has no dependency on it and makes no changes to it — so it works in any session, with or without other plugins installed.
...
3. **Synthesize with root-cause attribution** — group entries by context. A group whose entries are all `reported` is skipped: instead of re-deriving its root cause, the report gets a short "Already tracked" note naming the earlier report (and story, when known) that covered it. For every group with at least one `open` entry, and for each recurring or clearly fixable problem, attribute the root cause to either a named skill's `SKILL.md` (located under `~/.claude/plugins/cache/{marketplace}/{plugin}/{version}/skills/{skill}/`, searched by name across marketplaces/plugins) with the specific instruction quoted, or a named `CLAUDE.md` (user-global or repo-level) with the missing/ignored rule identified. Never a generic observation.
4. **Write the report** — render a standalone, self-contained, light/dark-aware HTML report (no external assets, no network calls) to `~/.claude/reflection/reports/{ISO-timestamp}.html`; then update the log in place, setting each written-up group's entries to `reported (report: {path}, at: {timestamp})`. Already-tracked groups are left untouched.
